  ...
  useEffect(() => {
    if (property) {
      if (property === "new-project") {
        ...
        property === "all-schedule-a-visit" ||
        property === "all-inquire-a-property"
      ) {
        property = "all";
      }
  ...
  const filteredProperties = properties.filter((property) => {
    ...
    const matchesStatus =
      ...
      (statusFilter.toLowerCase().replace(/-/g, " ").trim() === "new" &&
        (property.status.toLowerCase().trim() === "new" ||
          property.status.toLowerCase().trim() === "new project")) ||
      statusFilter.toLowerCase().replace(/-/g, " ").trim() ===
        property.status.toLowerCase().replace(/-/g, " ").trim();
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8">
          {filteredProperties.length > 0 ? (
            filteredProperties.map((residence, index) => {
              ...
              const badgeColor =
                statusColors[residence.status] || "bg-gray-500";

              return (
                <Link
                  key={index}
                  href={`/user/property/${residence.id}`}
                  passHref
                >
                  <Card className="relative rounded-xl shadow-lg border border-gray-200 hover:shadow-2xl transition duration-300 cursor-pointer">
                    <CardHeader className="p-0 relative">
                      <span
                        className={`absolute top-4 right-4 text-white text-xs font-semibold px-3 py-1 rounded-md ${badgeColor}`}
                      >
                        {residence.status}
                      </span>
                      <img
                        src={`${API_BASE_URL}${residence.image}`}
                        alt={residence.name}
                    ...
                    <CardContent className="text-center">
                      ...
                      <p className="text-gray-700 text-sm">
                        {residence.developmentType}
                      </p>
                      <p className="text-gray-700 text-sm">
                        Lot Area: {residence.lotArea} sqm
                      </p>

                      <p className="text-gray-700 text-sm">
                        Units:{" "}
                        {Array.isArray(residence.units)
                          ? residence.units.join(", ")
                          : "N/A"}
                      </p>
