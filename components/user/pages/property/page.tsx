...
export default function Property({ id }: { id: string }) {
  ...
  const property = useSelector((state: RootState) => state.propertyData.selectedProperty);
...
<div className="mt-6">
  ...
  <p className="text-xl font-semibold text-black mt-2">
    ₱ {Number(property.priceRange.split(" - ")[0]).toLocaleString()} - ₱ {Number(property.priceRange.split(" - ")[1]).toLocaleString()}
  </p>

  {/* ✅ Units - Parse it first before joining */}
  <p className="text-md text-gray-700 mt-2">
    <strong>Available Units:</strong>{" "}
    {Array.isArray(property.units) ? property.units.join(", ") : JSON.parse(property.units).join(", ")}
  </p>
</div>
      ...
      <div className="mt-6">
        ...
        <ul className="list-disc list-inside text-gray-700 mt-2 space-y-1">
          {JSON.parse(property.features).map((feature: { name: string }, index: number) => (
        ...
        <div className="w-full">
          <p className="text-lg font-semibold text-gray-800 mb-2">Master Plan:</p>
          <img 
      ...
      <div className="mt-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-3">
          {(typeof property.amenities === "string" ? JSON.parse(property.amenities) : property.amenities)?.map(
            (amenity: { name: string; image: string }, index: number) => (
