  ...
  return (
    <div className="w-64 bg-black text-white flex flex-col p-5 space-y-6">
      ...
      <div className="flex justify-center -ml-16">
        <Image src="/logo/megaworld-white.png" alt="Megaworld Logo" width={150} height={50} />
      </div>
      ...
      <nav className="space-y-3">
        {navItems.map((item) => (
          <div key={item.title}>
            <button
              onClick={() => item.submenu.length ? toggleMenu(item.title) : setActiveTab(item.title)}
            ...
            >
              ...
              {item.submenu.length > 0 && <FaChevronDown className={`transition ${openMenu === item.title ? "rotate-180" : ""}`} />}
            </button>
            {openMenu === item.title && item.submenu.length > 0 && (
              <div className="ml-4 space-y-2">
                {item.submenu.map((sub) => (
                  typeof sub === "string" ? (
                    <button key={sub} className="block w-full text-left p-2 rounded-md hover:bg-gray-700 text-sm" onClick={() => setActiveTab(sub)}>
                      {sub}
                    </button>
                  ) : (
                    <button key={sub.label} className="block w-full text-left p-2 rounded-md hover:bg-gray-700" onClick={() => setActiveTab(sub.label)}>
                      {sub.label}
                    </button>
                  )
                ))}
