...
import { fetchProperties, deleteProperty } from "@/app/redux/services/propertyService"; // Redux actions
...
export default function PropertyPage() {
  ...
  const properties = useSelector((state: RootState) => state.propertyData.properties); // Get properties from Redux
  const { loading, error } = useSelector((state: RootState) => state.propertyData); // Get loading and error states from Redux
  ...
  const columns = [
    ...
    { name: "Parking Lots", selector: (row: Property) => row.parkingLots || "N/A", sortable: true },
    {
      name: "Master Plan",
      cell: (row: Property) => (
        <img src={`${API_BASE_URL}${row.masterPlan}`} alt="Master Plan" className="w-12 h-12 rounded-md" />
      ),
      sortable: false,
    },
    {
      name: "Image",
      cell: (row: Property) => (
        <img src={`${API_BASE_URL}${row.image}`} alt={row.name} className="w-12 h-12 rounded-md" />
      ),
      sortable: false,
    },
    {
