...
import {
  fetchRealEstateTips,
  deleteRealEstateTip,
} from "@/app/redux/services/realestateTipsService";
...
export default function RealEstateTipsPage() {
  ...
  const { tips, loading, error } = useSelector(
    (state: RootState) => state.realEstateTipsData
  );
 const API_BASE_URL = process.env.NEXT_PUBLIC_BACKEND_URL;
  ...
  const handleDeleteTip = async () => {
    if (tipToDelete) {
      await dispatch(deleteRealEstateTip(tipToDelete.id));
  ...
  return (
    <div className="w-full mx-auto p-6 bg-white shadow-md rounded-lg overflow-visible">
      ...
      <AddModal
        ...
        fetchData={() => dispatch(fetchRealEstateTips())}
        itemType="realEstateTips"
      />
      ...
      {deleteModalOpen && tipToDelete && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-md shadow-md w-80">
            ...
            <p className="mb-4">
              Are you sure you want to delete the tip "{tipToDelete.title}"?
            </p>
