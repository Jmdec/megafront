...
import {
  fetchClosedDeals,
  deleteClosedDeal,
} from "@/app/redux/services/closedDealService"; // Import Redux actions
...
export default function ClosedDealsPage() {
  ...
  const { closedDeals, loading } = useSelector((state: RootState) => state.closedDealsData);
 ...
 const API_BASE_URL = process.env.NEXT_PUBLIC_BACKEND_URL;
  ...
  const handleDeleteDeal = async () => {
    if (dealToDelete) {
      await dispatch(deleteClosedDeal(dealToDelete.id));
  ...
  return (
    <div className="w-full mx-auto p-6 bg-white shadow-md rounded-lg overflow-visible">
      ...
      <AddModal
        ...
        fetchData={() => dispatch(fetchClosedDeals())}
        itemType="closedDeal"
      />

      {/* Edit Closed Deal Modal */}
      {editingDeal && (
        <EditModal
          modalOpen={true}
          closeModal={() => setEditingDeal(null)}
          item={editingDeal}
          fetchData={() => dispatch(fetchClosedDeals())}
          itemType="closedDeal"
        />
      ...
      {deleteModalOpen && dealToDelete && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-md shadow-md w-80">
            ...
            <p className="mb-4">
              Are you sure you want to delete the closed deal "{dealToDelete.title}"?
            </p>
